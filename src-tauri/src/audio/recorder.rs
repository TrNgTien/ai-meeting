...
impl MeetingRecorder {
    ...
    /// The name every file of this recording is built from —
    /// `meeting-YYYYmmdd-HHMMSS`.
    pub fn stem(&self) -> &str {
        &self.stem
    }

    /// What went wrong while opening the two sides, if anything. Readable while
    /// the recording runs so the UI can say "the meeting track is missing"
    /// *during* the meeting, when it can still be fixed.
    ...
    pub fn stop(mut self) -> Recording {
        ...
        // zero-length WAV would make the merge report an empty speaker. Say so
        // rather than dropping it silently — a meeting is not repeatable, and
        // "half of it is missing" has to be visible while it still matters.
        let mic_path = keep_if_recorded(
            self.mic_path.take(),
            mic_frames,
            "The microphone recorded nothing.",
            &mut self.warnings,
        );
        let system_path = keep_if_recorded(
            self.system_path.take(),
            system_frames,
            "No system audio was captured — was anything playing?",
            &mut self.warnings,
        );
...
/// Drop a track that captured nothing, and say why.
///
/// A zero-length WAV is not a track: transcribing it yields an empty speaker in
/// the merged conversation, which reads as "they said nothing" rather than "this
/// was never recorded". The file goes, and the reason takes its place on the
/// recording — a meeting cannot be repeated, so a missing side has to be
/// visible. A side that never opened at all is `None` here already and keeps the
/// more specific reason `start` recorded.
fn keep_if_recorded(
    path: Option<PathBuf>,
    frames_sec: f64,
    reason: &str,
