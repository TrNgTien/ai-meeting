...
# The desktop app has no Python in it: transcription, model management and
# recording are all native Rust, so there is nothing to package alongside the
# binary and the built .app is portable by construction (survives the repo
# moving or being deleted, or being copied to another Mac).
